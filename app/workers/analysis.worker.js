...
import { computeBoundsTree, disposeBoundsTree, SAH } from "three-mesh-bvh"
...
// Best Fit provádí desetitisíce nearest-surface dotazů. SAH je dražší na sestavení,
// ale vytváří kvalitnější strom pro opakované closest-point dotazy. Výsledek geometrie
// se nemění, pouze organizace trojúhelníků uvnitř BVH.
...
self.onmessage = async (event) => {
  ...
  if (message.type === "BEST_FIT") {
    ...
    try {
      ...
      if (!pair.targetGeometry.boundsTree) pair.targetGeometry.computeBoundsTree(ALIGNMENT_BVH_OPTIONS)
      ...
      const expectedAlgorithmError =
        ...
        errorMessage.includes("Moving model nemá dostatek prostorově rozložených bodů")
