...
function repairSolveLinear(matrix, vector) {
...
function repairFitSurface(boundary, basis) {
  const radius = Math.max(1e-5, basis.radius)
  const ata = Array.from({ length: 6 }, () => new Array(6).fill(0))
  const atb = new Array(6).fill(0)
  const addEquation = (row, rhs, weight = 1) => {
    for (let i = 0; i < 6; i++) {
  ...
  for (const sample of boundary) {
    ...
    const x = q.dot(basis.u) / radius
    const y = q.dot(basis.v) / radius
    const z = q.dot(basis.n) / radius
    addEquation([x*x, y*y, x*y, x, y, 1], z, 1)
    if (sample.normal) {
      const nn = sample.normal.dot(basis.n)
      if (Math.abs(nn) > 0.18) {
        const sx = -sample.normal.dot(basis.u) / nn
        const sy = -sample.normal.dot(basis.v) / nn
        addEquation([2*x, 0, y, 1, 0, 0], sx, 0.32)
        addEquation([0, 2*y, x, 0, 1, 0], sy, 0.32)
      }
    }
  }
  for (let i = 0; i < 6; i++) ata[i][i] += 1e-6
  const coeff = repairSolveLinear(ata, atb) || [0, 0, 0, 0, 0, 0]
  const evalSurface = (x, y) => coeff[0]*x*x + coeff[1]*y*y + coeff[2]*x*y + coeff[3]*x + coeff[4]*y + coeff[5]
  const normalAt = (x, y) => {
    const dx = 2*coeff[0]*x + coeff[2]*y + coeff[3]
    const dy = 2*coeff[1]*y + coeff[2]*x + coeff[4]
    return basis.u.clone().multiplyScalar(-dx).addScaledVector(basis.v, -dy).add(basis.n).normalize()
  }
  return { coeff, evalSurface, normalAt }
}
...
function repairPrepareBoundaryGuides(boundary) {
  ...
  // guideDirection vždy znamená "healthy side" – tedy směr z boundary do již
  // existujícího zdravého meshe. Chybějící strana otvoru je přesně opačný směr.
  // Používáme přímo centroid sousedních zdravých triangles, takže znaménko není
  // závislé na orientaci boundary ani na windingu normál.
  const rawDirections = boundary.map((sample, index) => {
    ...
    const healthyVector = sample.healthyHint?.clone?.().sub(sample.position) || new THREE.Vector3()
    healthyVector.addScaledVector(normal, -healthyVector.dot(normal))
    if (healthyVector.lengthSq() > 1e-12) return healthyVector.normalize()

    // Fallback jen pro patologický boundary vertex bez použitelného healthy hint.
  ...
  // Po vyhlazení znovu kontrolujeme znaménko vůči skutečné healthy straně.
  // Vyhlazení přes hodně členitou hranici jinak může lokálně otočit směr o 180°.
  return boundary.map((sample, index) => {
    const normal = sample.normal?.clone?.() || new THREE.Vector3(0, 0, 1)
    if (normal.lengthSq() < 1e-12) normal.set(0, 0, 1)
    normal.normalize()

    const direction = smoothed[index]?.clone?.() || rawDirections[index]?.clone?.() || new THREE.Vector3(1, 0, 0)
    direction.addScaledVector(normal, -direction.dot(normal))
    if (direction.lengthSq() < 1e-12) direction.copy(rawDirections[index] || new THREE.Vector3(1, 0, 0))
    direction.normalize()
    ...
    return { ...sample, guideDirection: direction }
  })
}
...
function classifyRepairBoundaryLoops(context, loops) {
  const list = Array.isArray(loops) ? loops.slice() : []
  if (!context || !list.length) return { visible: list, hiddenBottom: null }

  // Spodní otevřená boundary intraorálního scanu je typicky zdaleka nejdelší
  // otevřená smyčka celého modelu. Schováváme ji pouze při vysoké jistotě.
  const byPerimeter = list.slice().sort((a, b) => b.perimeter - a.perimeter)
  const largest = byPerimeter[0]
  const second = byPerimeter[1] || null
  const diagonal = Math.max(1e-6, Number(context.diagonal) || 0)
  const sizeRatio = largest.perimeter / diagonal
  const dominanceRatio = second
    ? largest.perimeter / Math.max(1e-6, second.perimeter)
    : Infinity

  const highConfidenceBottom =
    !largest.likely &&
    sizeRatio >= 2.15 &&
    (second ? dominanceRatio >= 2.35 : sizeRatio >= 2.8)

  if (!highConfidenceBottom) return { visible: list, hiddenBottom: null }

  return {
    visible: list.filter((hole) => hole.id !== largest.id),
    hiddenBottom: {
      ...largest,
      bottomConfidence: Math.min(
        1,
        0.72 +
          Math.min(0.18, (sizeRatio - 2.15) * 0.08) +
          Math.min(0.10, (dominanceRatio - 2.35) * 0.04)
      ),
    },
  }
...
function repairSampleBoundaryAtAngle(boundary, theta, angle) {
  ...
  const position = a.position?.clone?.().lerp(b.position || a.position, t) || new THREE.Vector3()
  const normal = a.normal?.clone?.().lerp(b.normal || a.normal, t)
  if (normal?.lengthSq?.() > 1e-12) normal.normalize()

  const guideDirection = a.guideDirection?.clone?.().lerp(b.guideDirection || a.guideDirection, t)
  if (guideDirection?.lengthSq?.() > 1e-12) {
    if (normal?.lengthSq?.() > 1e-12) guideDirection.addScaledVector(normal, -guideDirection.dot(normal))
    if (guideDirection.lengthSq() > 1e-12) guideDirection.normalize()
  }

  return {
    position,
    normal: normal || new THREE.Vector3(0, 0, 1),
    guideDirection: guideDirection || new THREE.Vector3(1, 0, 0),
...
function buildRepairPatchData(context, hole) {
  ...
  triangles.forEach((triangle) => {
    ...
    triangle.forEach((corner) => {
      if (corner.sourceNormal) expected.add(trimVec(corner.sourceNormal))
    })
...
// TEST (rounded shoulder V38): builds a per-vertex averaged face-normal map from a flat
// triangle-soup position array. Used to recover the real local surface tangent at the
// scan boundary so the new shoulder can leave the scan surface tangentially (G1) instead
// of kinking into a straight XZ/Y blend.
function cadBuildVertexNormalMap(trianglePositions, tolerance) {
  const map = new Map()
  const a = new THREE.Vector3(), b = new THREE.Vector3(), c = new THREE.Vector3()
  const ab = new THREE.Vector3(), ac = new THREE.Vector3(), normal = new THREE.Vector3()
  const source = trianglePositions || []
  for (let i = 0; i + 8 < source.length; i += 9) {
    a.set(source[i], source[i + 1], source[i + 2])
    b.set(source[i + 3], source[i + 4], source[i + 5])
    c.set(source[i + 6], source[i + 7], source[i + 8])
    ab.subVectors(b, a)
    ac.subVectors(c, a)
    ...
    normal.normalize()
    ;[a, b, c].forEach((v) => {
      const key = cadQuantizedVertexKey(v, tolerance)
...
function cadLookupVertexNormal(map, point, tolerance, fallback) {
  const key = cadQuantizedVertexKey(point, tolerance)
  const entry = map?.get(key)
  if (entry && entry.lengthSq() > 1e-10) return entry.clone().normalize()
  return fallback ? fallback.clone() : new THREE.Vector3(0, 1, 0)
}
...
// V37 – true triangle clipping against the finished Straight footprint.
//
// V36 confirmed that the final base footprint is the right geometric hard limit,
// but projecting existing scan vertices onto that limit collapsed many triangles onto
// vertical lines. V37 never moves a healthy source vertex. Instead, only triangles in
// a narrow band around the open scan boundary are classified in XZ against the final
// Straight footprint. Intersected triangle edges get NEW vertices at the actual
// footprint crossing, the outside fragment is discarded, and the inside fragment is
// retriangulated locally. The resulting open loop is extracted again from the clipped
// source mesh and becomes the exact start boundary for a freshly rebuilt shoulder strip.
function cadExactClipScanToBaseFootprint(trianglePositions, rawLoop, footprintLoop, diagonal) {
  ...
  if (source.length < 9 || raw.length < 4 || footprint.length < 4) {
    return {
      ...
      extractedBoundaryLoops: 0,
      extractedBoundaryPoints: raw.length,
      boundaryPerimeterBefore: cadClosedLoopPerimeter(raw),
      boundaryPerimeterAfter: cadClosedLoopPerimeter(raw),
    }
  }
  ...
  // A cell as wide as the clip band means that any point closer than clipBandWidth
  // can only live in the same or one of the 26 neighboring cells. This avoids the
  // expensive wide-radius spatial search that made the V36 experiment stall on large scans.
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    if (!bucket) {
      bucket = []
      boundaryGrid.set(key, bucket)
    }
    bucket.push(p)
  }

  // Coordinates in an STL/triangle soup are repeated between neighboring faces.
  // Cache all expensive point tests by a fine quantized coordinate key.
  const quant = 1 / Math.max(1e-6, safeDiagonal * 1.0e-7)
  const pointKey = (p) => `${Math.round(p.x * quant)}|${Math.round(p.y * quant)}|${Math.round(p.z * quant)}`
  const boundaryDistanceCache = new Map()
  const classificationCache = new Map()
  const intersectionCache = new Map()

  const distanceToRawBoundarySq = (point) => {
    const key = pointKey(point)
    const cached = boundaryDistanceCache.get(key)
    if (cached != null) return cached
    const cx = Math.floor(point.x * invBoundaryCell)
    const cy = Math.floor(point.y * invBoundaryCell)
    const cz = Math.floor(point.z * invBoundaryCell)
    let best = Infinity
    for (let dx = -1; dx <= 1; dx++) {
      for (let dy = -1; dy <= 1; dy++) {
        for (let dz = -1; dz <= 1; dz++) {
          ...
          for (const candidate of bucket) {
            const ddx = point.x - candidate.x
            const ddy = point.y - candidate.y
            const ddz = point.z - candidate.z
            const d2 = ddx * ddx + ddy * ddy + ddz * ddz
            if (d2 < best) best = d2
          }
        }
    ...
    boundaryDistanceCache.set(key, best)
    return best
  }
  ...
  const segmentFootprintHits = (a, b) => {
    ...
    const rx = b.x - a.x
    const rz = b.z - a.z
    const rLenSq = rx * rx + rz * rz
    if (rLenSq <= 1e-18) return hits
  ...
  const binaryCrossingFallback = (a, b, aInside) => {
    ...
    for (let iteration = 0; iteration < 24; iteration++) {
      const mid = (lo + hi) * 0.5
      const probe = a.clone().lerp(b, mid)
      const midInside = classifyPoint(probe).inside
      if (midInside === aInside) lo = mid
      else hi = mid
    }
    return (lo + hi) * 0.5
  }

  const intersectEdge = (a, b, aInside, bInside) => {
    const ka = pointKey(a), kb = pointKey(b)
    const undirectedEdgeKey = ka < kb ? `${ka}~${kb}` : `${kb}~${ka}`
    const hits = segmentFootprintHits(a, b)
    let chosen = null
    if (hits.length) {
      chosen = aInside && !bInside ? hits[0] : hits[hits.length - 1]
    }
    const cacheKey = `${undirectedEdgeKey}#${chosen?.segmentIndex ?? -1}`
    ...
    // When we know the exact footprint segment, snap XZ to that segment analytically.
    // Y stays the source-edge interpolation, so the operation is a true vertical prism cut.
    if (chosen) {
      const c = polygon[chosen.segmentIndex]
      const d = polygon[(chosen.segmentIndex + 1) % polygon.length]
      const vx = d.x - c.x, vz = d.y - c.y
      const len2 = vx * vx + vz * vz
      if (len2 > 1e-18) {
        const s = THREE.MathUtils.clamp(((point.x - c.x) * vx + (point.z - c.y) * vz) / len2, 0, 1)
        point.x = c.x + vx * s
        point.z = c.y + vz * s
      }
    }

    intersectionCache.set(cacheKey, point.clone())
    return point
  }

  const pushTriangle = (out, a, b, c, stats) => {
    const area2 = b.clone().sub(a).cross(c.clone().sub(a)).length()
    if (!Number.isFinite(area2) || area2 <= areaThreshold) {
      stats.removedDegenerateTriangles++
      return false
    }
  ...
  for (let offset = 0; offset + 8 < source.length; offset += 9) {
    ...
    const candidate = tri.some((p) => distanceToRawBoundarySq(p) <= clipBandSq)
    if (!candidate) {
      clipped.push(
    ...
    const inside = tri.map((p) => classifyPoint(p).inside)
    ...
    if (insideCount === 0) {
      // In the narrow open-edge band this triangle is entirely on the forbidden side
      // of the finished footprint. Remove it instead of collapsing it onto the wall.
    ...
    for (let edge = 0; edge < 3; edge++) {
      const next = (edge + 1) % 3
      const current = tri[edge]
      const following = tri[next]
      const currentInside = inside[edge]
      const nextInside = inside[next]
      if (currentInside) polygon3D.push(current.clone())
      if (currentInside !== nextInside) {
        polygon3D.push(intersectEdge(current, following, currentInside, nextInside))
      }
    }

    // Remove consecutive duplicate points caused by a triangle vertex already lying
    // exactly on the footprint boundary.
    const cleanPoly = []
    for (const point of polygon3D) {
      if (!cleanPoly.length || cleanPoly[cleanPoly.length - 1].distanceToSquared(point) > boundaryToleranceSq * 0.25) {
        cleanPoly.push(point)
      }
    }
    if (cleanPoly.length > 2 && cleanPoly[0].distanceToSquared(cleanPoly[cleanPoly.length - 1]) <= boundaryToleranceSq * 0.25) cleanPoly.pop()
    ...
    if (cleanPoly.length > 3) stats.splitTriangles++
    let generatedForSource = 0
    for (let i = 1; i + 1 < cleanPoly.length; i++) {
      if (pushTriangle(clipped, cleanPoly[0], cleanPoly[i], cleanPoly[i + 1], stats)) generatedForSource++
    }
    stats.generatedTriangles += generatedForSource
  }

  // The intersection operation creates a genuinely new source-mesh boundary. Recover
  // that loop from topology instead of trying to predict it from the old vertex order.
  const extracted = cadExtractBoundaryLoopsRobust(clipped)
  let newBoundary = extracted?.loops?.[0]?.points?.map((p) => p.clone()) || raw.map((p) => p.clone())

  // Preserve orientation AND phase relative to the original conditioned loop. The
  // Straight target was built from rawLoop before clipping, so a phase shift here would
  // connect the palatal side to a buccal target even though both loops are individually valid.
  const rawArea = cadSignedAreaXZ(raw)
  if (newBoundary.length >= 4 && cadSignedAreaXZ(newBoundary) * rawArea < 0) newBoundary.reverse()
  if (newBoundary.length >= 4) {
    const anchor = raw[0]
    let anchorIndex = 0
    let anchorDistanceSq = Infinity
    for (let i = 0; i < newBoundary.length; i++) {
      const point = newBoundary[i]
      const dx = point.x - anchor.x
      const dz = point.z - anchor.z
      const dy = (point.y - anchor.y) * 0.18
      const d2 = dx * dx + dz * dz + dy * dy
      if (d2 < anchorDistanceSq) {
        anchorDistanceSq = d2
        anchorIndex = i
      }
    }
    if (anchorIndex > 0) newBoundary = newBoundary.slice(anchorIndex).concat(newBoundary.slice(0, anchorIndex))
  }

  return {
    trianglePositions: clipped,
    ...
    intersectionVertices: intersectionCache.size,
    removedDegenerateTriangles: stats.removedDegenerateTriangles,
    extractedBoundaryLoops: extracted?.loops?.length || 0,
    extractedBoundaryPoints: newBoundary.length,
    boundaryPerimeterBefore: cadClosedLoopPerimeter(raw),
    boundaryPerimeterAfter: cadClosedLoopPerimeter(newBoundary),
    boundaryTolerance,
  }
}
...
// V37: Exact footprint clip + rebuilt transition strip.
//
// V36 proved that the finished Straight footprint is the correct hard limit, but
// vertex projection damaged topology. V37 returns to the clean V35 conditioned scan,
// performs a TRUE triangle/footprint cut in the narrow boundary band, extracts the
// resulting intersection loop, then rebuilds the shoulder from uniformly sampled
// intermediate rings. Healthy source vertices are never snapped to the base wall.
function cadBuildSolidBaseGeometry(sourceObject, viewerRoot, totalHeight, arch = "lower", boundaryOverride = null) {
  ...
  // V37: physically cut the conditioned source triangles with the final Straight
  // footprint. No source vertex is projected or collapsed. The returned boundary is
  // the actual new open edge of the clipped source mesh.
  ...
  // TEST (rounded shoulder V38): recover the real scan-surface normal at every boundary
  // vertex so the new shoulder profile can leave the scan tangentially instead of kinking.
  // Tolerance is deliberately coarse (real mm, not the ultra-fine welding tolerance used for
  // topology) since we only need to re-find the same coordinate that cadExactClipScanToBaseFootprint
  // already produced, not weld distinct nearby vertices together.
  const boundaryNormalTolerance = Math.max(0.01, boundaryData.diagonal * 2e-4)
  const boundaryNormalMap = cadBuildVertexNormalMap(trianglePositions, boundaryNormalTolerance)
  const boundaryNormalFallback = new THREE.Vector3(0, isUpper ? -1 : 1, 0)
  const rawLoopNormals = rawLoop.map((p) =>
    cadLookupVertexNormal(boundaryNormalMap, p, boundaryNormalTolerance, boundaryNormalFallback)
  )
  const sampleRawNormal = (uu) =>
    cadSampleClosedLoopArc(rawLoopNormals, rawArcTable, uu).normalize()
  const rawTangentEpsilon = Math.max(1e-4, 0.6 / Math.max(1, rawArcTable.total))
  const sampleRawTangent = (uu) => {
    const a = cadSampleClosedLoopArc(rawLoop, rawArcTable, uu - rawTangentEpsilon)
    const b = cadSampleClosedLoopArc(rawLoop, rawArcTable, uu + rawTangentEpsilon)
  ...
  // TEST (V38): switch between the old linear XZ/Y blend ("legacy") and a tangent-matched
  // Hermite profile curve ("hermite") that produces a genuinely rounded shoulder instead of
  // relying on a smootherstep kink + fairing to fake roundness. Flip this back to "legacy"
  // to instantly A/B compare against the previous behaviour.
  const shoulderProfileMode = "hermite"
  // How far the tangent handles reach toward the middle of the shoulder, as a fraction of
  // the local start->end chord length. Higher = rounder / more bulge, lower = closer to a
  // straight bridge. Good starting range: 0.35 (subtle) .. 0.75 (very rounded).
  const shoulderRoundness = 0.55
  const shoulderEndRoundness = shoulderRoundness * 0.85
  ...
  const evaluateDirectBridge = (u, t) => {
    ...
    const end = new THREE.Vector3(smooth.x, targetY, smooth.z)

    let point
    if (shoulderProfileMode === "hermite") {
      // Real scan-surface exit direction: perpendicular to the boundary's own circumferential
      // tangent, lying in the surface's tangent plane (cross of loop-tangent and surface normal).
      const normal = sampleRawNormal(uu)
      const loopTangent = sampleRawTangent(uu)
      const exitDir = loopTangent.clone().cross(normal)
      if (exitDir.lengthSq() < 1e-10) exitDir.set(0, transitionDirection, 0)
      else exitDir.normalize()
      // Orient consistently toward the base side (avoids random flips from normal winding).
      if (exitDir.dot(end.clone().sub(start)) < 0) exitDir.negate()

      const chordLength = Math.max(1e-6, start.distanceTo(end))
      const startTangent = exitDir.multiplyScalar(chordLength * shoulderRoundness)
      // The base wall is essentially vertical, so the curve should arrive vertically too.
      const endTangent = new THREE.Vector3(0, transitionDirection, 0)
        .multiplyScalar(chordLength * shoulderEndRoundness)
      ...
      point.addScaledVector(start, h00)
      point.addScaledVector(startTangent, h10)
      point.addScaledVector(end, h01)
      point.addScaledVector(endTangent, h11)
    } else {
  ...
  return {
    ...
    stats: {
      ...
      transitionDepth,
      transitionEndOffset,
      transitionMode: "exact-footprint-triangle-clip-rebuilt-shoulder-v37",
      transitionRemeshEnabled: true,
      transitionRemeshMethod: "exact-triangle-footprint-clip-plus-new-intersection-boundary-plus-uniform-arc-rings-plus-local-edge-flip-fairing",
      wallAcquireStart,
      bridgeIntervals,
...
export default function ClientPage({ forceCadMode = false, forceAlignmentDemo = false } = {}) {
  ...
  const [editorCapabilities, setEditorCapabilities] = useState({ canSaveAlignedToCase: false, canSaveTrimmedToCase: false, canSaveRepairedToCase: false })
  ...
  const [repairStage, setRepairStage] = useState("model") // model | detect | paint | preview | result
  ...
  const [repairBrushFactor, setRepairBrushFactor] = useState(0.035)
  const [repairBrushCursor, setRepairBrushCursor] = useState(null)
  ...
  useEffect(() => {
    ...
    const onMessage = (event) => {
      ...
      setEditorCapabilities({
        canSaveAlignedToCase: !!payload.canSaveAlignedToCase,
        canSaveTrimmedToCase: !!payload.canSaveTrimmedToCase,
        canSaveRepairedToCase: !!payload.canSaveRepairedToCase,
      })
    }
  ...
  const openRepairMode = useCallback(() => {
    ...
    setRepairMode(true)
    setRepairMessage("Vyberte model. Auto najde opravitelné otevřené hranice a dovolí vybrat více děr najednou; spodní otevřenou část scanu automaticky ignorujeme.")
  ...
  const selectRepairHole = useCallback((holeId, contextValue = repairContext, holesValue = repairHoles) => {
    ...
    if (repairVariant === "manual") {
      ...
      setRepairSelectedHoleIds([hole.id])
      setRepairPreviewPatch(null)
      setRepairStage("preview")
      setRepairMessage("Otvor je vybraný. Jasně modrý obrys ve scéně označuje boundary, která se opraví po potvrzení.")
      return
    }

    const alreadySelected = repairSelectedHoleIds.includes(hole.id)
    const next = alreadySelected
      ? repairSelectedHoleIds.filter((id) => id !== hole.id)
      : [...repairSelectedHoleIds, hole.id]

    setRepairSelectedHoleIds(next)
    setRepairSelectedHoleId(next.includes(hole.id) ? hole.id : (next[next.length - 1] || ""))
    setRepairPreviewPatch(null)
    setRepairStage(next.length ? "preview" : "detect")
  ...
  const changeRepairVariant = useCallback((variant) => {
    ...
    setRepairVariant(variant)
    setRepairPaintedTriangles(new Set())
    setRepairBrushCursor(null)
    setRepairSelectedHoleId("")
    setRepairSelectedHoleIds([])
    setRepairPreviewPatch(null)
    if (!repairContext || !repairSelection) {
      setRepairStage("model")
      setRepairMessage(variant === "manual" ? "Vyberte model a potom zvýrazňovačem označte oblast kolem díry nebo trhliny." : "Vyberte model pro automatickou analýzu otvorů.")
      return
    }
    if (variant === "manual") {
      setRepairStage("paint")
      setRepairMessage("LMB = malovat oblast · RMB = kamera · velikost štětce nastavíte posuvníkem. Označte okolí problému, nemusíte přesně obkreslovat hranu.")
    } else {
      setRepairStage("detect")
      setRepairMessage(repairHoles.length
        ? "Vyberte jednu nebo více děr. Jasně modrý obrys označí výběr; CGAL se spustí až po potvrzení."
        : "Automatická analýza nenašla opravovatelný uzavřený otvor.")
    }
  }, [repairContext, repairSelection, repairHoles])


  const paintRepairHit = useCallback((hit) => {
    if (!repairContext || !hit) return
    const radius = repairContext.diagonal * repairBrushFactor
    const added = collectRepairBrushTriangles(repairContext, hit, radius)
    if (!added.size) return
    setRepairPaintedTriangles((previous) => {
      const next = new Set(previous)
      added.forEach((triIndex) => next.add(triIndex))
      return next
    })
  }, [repairContext, repairBrushFactor])

  const repairCursorFromHit = useCallback((hit) => {
    if (!repairContext || !hit) return null
    const triangle = repairContext.triangles[hit.triangleIndex]
    if (!triangle) return null
    const a = trimVec(triangle.corners[0].sourcePos)
    const b = trimVec(triangle.corners[1].sourcePos)
    const c = trimVec(triangle.corners[2].sourcePos)
    ...
    return { point: hit.point, quaternion }
  }, [repairContext])

  const handleRepairSurfaceDown = useCallback((url, event) => {
    if (!repairMode || repairVariant !== "manual" || repairStage !== "paint" || repairBusy || url !== repairSelection) return
    if ((event.button ?? event.nativeEvent?.button) !== 0) return
    const hit = resolveTrimHit(repairContext, modelObjectsRef.current[url], event)
    if (!hit) return
    event.stopPropagation?.()
    event.nativeEvent?.preventDefault?.()
    event.nativeEvent?.stopImmediatePropagation?.()
    setRepairPainting(true)
    if (trackballRef.current) trackballRef.current.enabled = false
    setRepairBrushCursor(repairCursorFromHit(hit))
    paintRepairHit(hit)
  }, [repairMode, repairVariant, repairStage, repairBusy, repairSelection, repairContext, repairCursorFromHit, paintRepairHit])

  const handleRepairSurfaceMove = useCallback((url, event) => {
    if (!repairMode || repairVariant !== "manual" || repairStage !== "paint" || repairBusy || url !== repairSelection || !repairContext) return
    const hit = resolveTrimHit(repairContext, modelObjectsRef.current[url], event)
    if (!hit) return
    setRepairBrushCursor(repairCursorFromHit(hit))
    if (!repairPainting) return
    event.stopPropagation?.()
    const now = typeof performance !== "undefined" ? performance.now() : Date.now()
    if (now - repairPaintLastAtRef.current < 34) return
    repairPaintLastAtRef.current = now
    paintRepairHit(hit)
  }, [repairMode, repairVariant, repairStage, repairBusy, repairSelection, repairContext, repairCursorFromHit, repairPainting, paintRepairHit])

  const handleRepairSurfaceOut = useCallback(() => {
    if (!repairPainting) setRepairBrushCursor(null)
  }, [repairPainting])
  ...
  const clearRepairPaint = useCallback(() => {
    ...
    setRepairPreviewPatch(null)
    setRepairStage("paint")
    setRepairMessage("Označení bylo vymazáno. Zvýrazňovačem označte oblast kolem problému.")
  }, [])

  const findRepairInPaint = useCallback(() => {
    if (!repairContext || !repairPaintedTriangles.size) {
      setRepairMessage("Nejdřív zvýrazňovačem označte oblast kolem otvoru.")
      return
    }
    let best = null
    for (const hole of repairHoles) {
      let overlap = 0
      hole.triangleIndices.forEach((triIndex) => { if (repairPaintedTriangles.has(triIndex)) overlap++ })
      const score = overlap / Math.max(1, hole.triangleIndices.length)
      if (overlap > 0 && (!best || score > best.score)) best = { hole, score, overlap }
    }
    if (!best) {
    ...
    setRepairMessage("Otvor v označené oblasti byl nalezen. Jasně modrý obrys ukáže boundary; CGAL se spustí až po potvrzení opravy.")
  ...
  }, [repairSelectedHoleId, applyRepairHoles])

  const repairSelectedAutoHoles = useCallback(() => {
    if (!repairSelectedHoleIds.length) {
      setRepairMessage("Nejdřív vyberte alespoň jednu díru.")
      return
    }
  ...
  const clearRepairHoleSelection = useCallback(() => {
    ...
    setRepairSelectedHoleId("")
    setRepairStage("detect")
    setRepairMessage("Výběr byl zrušen. Kliknutím na čísla děr vyberte jednu nebo více oblastí.")
  }, [])


  const continueRepair = useCallback(() => {
    const url = repairSelection
    if (!url) return
    setRepairContext(null)
    setRepairHoles([])
    setRepairSelectedHoleId("")
  ...
  }, [repairSelection, selectRepairModel])
  ...
  const createRepairedExport = useCallback(async (url) => {
    ...
    const file = files.find((item) => item.url === url)
    const sourceObject = modelObjectsRef.current[url]
    if (!info || !file || !sourceObject || !rootGroupRef.current) throw new Error("Pro tento model zatím není dokončená Oprava sítě.")
    const ext = inferExt(file.rawName || file.name || file.url)
    if (!["stl", "ply", "obj"].includes(ext)) throw new Error(`Export .${ext || "?"} není podporovaný.`)
    setRepairExportBusyUrl(url)
    let exportObject = null
    try {
      exportObject = buildBakedAlignedExportObject(sourceObject, rootGroupRef.current)
      const blob = await alignedObjectToBlob(exportObject, ext)
      return {
        ...
        ext,
        mimeType: alignedExportMime(ext),
        name: makeRepairedExportName(file, !!alignedExportsByUrl[url], !!trimmedExportsByUrl[url]),
      ...
      disposeAlignedExportObject(exportObject)
      setRepairExportBusyUrl("")
    }
  }, [repairedExportsByUrl, files, alignedExportsByUrl, trimmedExportsByUrl])

  const downloadRepairedModel = useCallback(async (url) => {
    try {
      ...
      console.error("Repair export download error:", error)
      setRepairMessage(error?.message || "Opravený model se nepodařilo exportovat.")
    }
  ...
  const saveRepairedModelToCase = useCallback(async (url, options = {}) => {
    ...
    try {
      ...
      const buffer = await result.blob.arrayBuffer()
      window.parent.postMessage({
        type: "ARTHETIC_SAVE_REPAIRED_MODEL",
        payload: {
          name: result.name,
          ext: result.ext,
          mimeType: result.mimeType,
  ...
  const slidersContent = fatal ? (
    ...
    <>
      {files.map((f, i) => {
        ...
        const repairedInfo = repairedExportsByUrl[f.url] || null
          ...
          <div key={`${f.url}-${i}`} className="control-row" style={{
            ...
            {repairedInfo && (
              <div style={{
                gridColumn: "1 / -1", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8,
                marginTop: 1, padding: "7px 8px", borderRadius: 8,
                background: "rgba(59,130,246,.045)", border: "1px solid rgba(96,165,250,.13)",
              }}>
                <div style={{ minWidth: 0 }}>
                  <div style={{ color: "#bfdbfe", fontSize: 8.6, fontWeight: 760, letterSpacing: ".02em" }}>REPAIRED · OPRAVENO</div>
                  <div style={{ marginTop: 2, color: "#707070", fontSize: 8.2 }}>{repairedInfo.count || 1} {Number(repairedInfo.count) === 1 ? "opravený otvor" : "opravené otvory"} · {repairedInfo.mode === "manual" ? "ručně" : "auto"}</div>
                </div>
                <div style={{ display: "flex", alignItems: "center", gap: 5, flex: "0 0 auto" }}>
                  <button type="button" onClick={() => downloadRepairedModel(f.url)} disabled={repairExportBusyUrl === f.url}
  ...
  const topBarRight = (!isMobile || heatmapMenuOpen || comparisonMenuOpen) && (
    <div style={{
      ...
      <div style={{ width: dicomLayoutActive ? 120 : 270, display: isMobile ? "none" : "block" }}>
        <button
          onClick={openRepairMode}
          disabled={analysisEligibleFiles.length < 1}
          style={{ ...viewerToolbarButtonStyle(analysisEligibleFiles.length < 1), opacity: analysisEligibleFiles.length < 1 ? 0.45 : 1 }}
          title="Automaticky nebo ručně vyhledat a uzavřít díry a trhliny ve scanu"
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
            <path d="M4 12c0-4.4 3.6-8 8-8s8 3.6 8 8-3.6 8-8 8"/>
            <path d="M8.2 12.2c0-2.1 1.7-3.8 3.8-3.8 1.5 0 2.8.8 3.4 2"/>
            <path d="M12 20H5v-7"/>
            <path d="M5 20l5-5"/>
          </svg>
          Oprava sítě
  ...
  const repairWorkspace = repairMode && !(cadStandalone && cadStage === "repair") && (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, minWidth: 0 }}>
        <div style={{ minWidth: 0, flex: "1 1 auto" }}>
          <div style={{ display: "flex", alignItems: "baseline", gap: 6, flexWrap: "wrap" }}>
            <span style={{ display: "inline-flex", alignItems: "baseline", gap: 0, fontSize: 13 }}><span style={{ fontWeight: 850 }}>ART</span><span style={{ fontWeight: 300 }}>HETIC</span></span>
            <span style={{ color: "#d7d7d7", fontSize: 13, fontWeight: 340 }}>Oprava sítě</span>
            {repairSelectedFile && <span style={{ marginLeft: 4, color: "#777", fontSize: 9.2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{stripExt(repairSelectedFile.rawName || repairSelectedFile.name)}</span>}
          </div>
          <div style={{ marginTop: 7, display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
            {[
              ["model", "Model"], ["work", repairVariant === "manual" ? "Oblast" : "Analýza"], ["preview", "Výběr"], ["result", "Výsledek"],
            ].map(([key, label], index) => {
              const active = key === "model" ? repairStage === "model" : key === "work" ? ["detect","paint"].includes(repairStage) : key === "preview" ? repairStage === "preview" : repairStage === "result"
              const done = key === "model" ? !!repairSelection : key === "work" ? (!!repairContext && repairStage !== "model") : key === "preview" ? (repairStage === "preview" || repairStage === "result") : repairStage === "result"
              return <React.Fragment key={key}>
                <div style={analysisStepChipStyle(active, done)}>{done && <span style={{ color: "#86efac" }}>✓</span>}<span>{label}</span></div>
                {index < 3 && <div style={{ width: 12, height: 1, background: "rgba(255,255,255,.07)" }} />}
              </React.Fragment>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <div style={{ display: "inline-flex", padding: 3, borderRadius: 9, background: "rgba(255,255,255,.025)", border: "1px solid rgba(255,255,255,.07)" }}>
      ...
      {repairStage === "model" && (
        <div style={{ display: "grid", gridTemplateColumns: "minmax(220px,1fr) auto", gap: 10, alignItems: "center" }}>
          <AlignmentModelDropdown badge="R" value={repairSelection || ""} files={analysisEligibleFiles} otherValue="" onChange={selectRepairModel} />
          <span style={{ color: "#777", fontSize: 9.2 }}>Vyberte v menu nebo klikněte přímo na model ve scéně.</span>
        </div>
      )}
      ...
      {repairVariant === "manual" && repairStage === "paint" && (
        <div style={{ display: "grid", gridTemplateColumns: "minmax(250px,1fr) auto", gap: 12, alignItems: "center" }}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}>
            <button type="button" onClick={clearRepairPaint} disabled={!repairPaintedTriangles.size} style={{ height: 31, padding: "0 9px", borderRadius: 8, border: "1px solid rgba(255,255,255,.08)", background: "rgba(255,255,255,.03)", color: repairPaintedTriangles.size ? "#aaa" : "#555", fontSize: 9, fontWeight: 680, cursor: repairPaintedTriangles.size ? "pointer" : "default" }}>Vymazat paint</button>
            <button type="button" onClick={findRepairInPaint} disabled={!repairPaintedTriangles.size || repairBusy} style={{ height: 31, padding: "0 10px", borderRadius: 8, border: "1px solid rgba(96,165,250,.20)", background: "rgba(59,130,246,.075)", color: repairPaintedTriangles.size ? "#bfdbfe" : "#607087", fontSize: 9, fontWeight: 710, cursor: repairPaintedTriangles.size ? "pointer" : "default" }}>Najít v označení</button>
          </div>
        </div>
      )}
    ...
    <div className="stage" style={{ position: "relative", width: "100vw", height: "100vh", background: "black", overflow: "hidden" }}>
      ...
      >
        ...
        <group ref={rootGroupRef}>
              ...
              <AnyModel
                ...
                onAlignmentSelect={alignmentMode && alignmentStep === "models" && !alignmentModelsSelected && !alignmentBusy && (!alignmentHasA || f.url !== alignmentPair.aUrl)
                  ...
                  : trimMode && trimStage === "model" && !trimBusy
                    ? selectTrimModel
                    : repairMode && repairStage === "model" && !repairBusy
                      ? selectRepairModel
                      : null}
          ...
          {repairMode && repairContext && repairSelection && repairStage !== "result" && (
            <RepairOverlay
              context={repairContext}
              modelMatrix={modelTransforms[repairSelection]}
              holes={repairVariant === "auto"
                ? repairHoles
        ...
        {!alignmentMode && !trimMode && !repairMode && dicomVolume && dicomSettings.viewMode !== "only2d" && (
          <DicomVolume
            volume={dicomVolume}
            settings={dicomSettings}
            interactive={false}
        ...
        {!alignmentMode && !trimMode && !repairMode && clippingEnabled && (!isMobile || dicomSettings.viewMode === "only2d") && (
        ...
        {!alignmentMode && !trimMode && !repairMode && clippingEnabled && !isMobile && activePlaneGroup && (
          <>
            <TransformControls
